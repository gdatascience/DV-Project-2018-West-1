...
ggplot() + 
  ...
  geom_point(data = street_lights %>% 
               filter(!is.na(Inspect_Date2)) %>%
               filter(year(Inspect_Date2) > 1971),
             aes(x = Lon, y = Lat, color = year(Inspect_Date2), alpha = 0.5)) +
  scale_color_gradient2(low = "red", high = "green", midpoint = 2008) +
  coord_fixed(1.3) + 
  theme_void()
```
